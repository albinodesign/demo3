...
- `src/content/` — **der einzige Ort für Inhalte**:
  ...
  - `cms.manifest.json` — kundeneditierbare Felder, **Version 2**: `{ "version": 2, "features": { "blog": false }, "sections": [...] }`; Sektionen gruppieren Felder (`id`, `label` DE, `type`, `file`, `path`, `maxLength`)
...
4. **Iframe-Preview:** Zwei Header-Regeln in `vercel.json`: Auf `klarwerk-fenster.de` (`has`-Bedingung auf Host) gilt `frame-ancestors 'self'`; überall sonst (Preview/localhost) `frame-ancestors 'self' http://localhost:* https://*.vercel.app`. **Niemals** `X-Frame-Options: DENY` hinzufügen. Die Preview-Bridge in `BaseLayout.astro` läuft nur im Iframe und ist **zwei-wege**: eingehend `CMS_FIELD_UPDATE` (aktualisiert alle Vorkommen via `querySelectorAll`) und `CMS_SELECT_MODE`, ausgehend `CMS_FIELD_SELECT` bei Klick auf ein markiertes Feld (inkl. Hover-/Selektions-Outline). Daher müssen `data-cms-field`-Marker an **allen** Vorkommen eines Werts stehen (z.B. `site.phone` in Header, Footer, FinalCta, Kontakt).
